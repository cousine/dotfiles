...
---@param msg? string
function M.show(position, msg)
  local prompt = msg and " " .. msg
  ...
  local options = vim.fn.extend(default_position, position or {})
  ...
  spinner_timer:start(
    ...
    vim.schedule_wrap(function()
      if vim.api.nvim_buf_is_valid(spinner_buf) then
        vim.api.nvim_buf_set_lines(spinner_buf, 0, -1, false, { config.spinner_frames[spinner_index] .. prompt })
      end
...
function M.hide(show_msg)
  if spinner_timer then
    ...
    if config.show_notification and show_msg then
