run_segment() {
  ...
  do
    timing=${prayers[$i]}
    timing_stamp="$(date --date=${timing} +%s)"
  ...
  next_prayer_stamp=$(date --date=${prayers[$next_prayer]} +%s)
  countdown=$(date -u -d "0 $next_prayer_stamp seconds - $current_timestamp seconds" +"%H:%M")
